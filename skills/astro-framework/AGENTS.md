...
**Version:** 3.0.0 | **Astro 6.x** | **Updated:** 2026-06-20 | **Author**: elkaix
...
| **Output modes** | `static` (default; opt routes into SSR with `prerender = false` + adapter) or `server` (all SSR; opt out with `prerender = true`). **No `hybrid`** mode |
...
Mostly static + a few dynamic routes? ──> output: 'static' (default) + adapter
  Opt those routes into on-demand rendering: export const prerender = false
Mostly dynamic? ──> output: 'server' + adapter
  Opt static routes out: export const prerender = true
(No output: 'hybrid' mode — per-route prerender covers this.)
```
...
- **NEVER** import Zod from `zod` — use `astro/zod`

---

## Astro 6 Notes (Post-Training Knowledge)

**Astro 6 baseline:** Node 22+ (Node 18 and 20 dropped), Vite 7, Zod 4 (via `astro/zod`).

### New & stable in Astro 6

These graduated from experimental flags to stable in Astro 6:

- **Content Security Policy** — `security.csp` config auto-generates script/style hashes
- **Fonts API** — built-in font optimization (self-hosted + provider fonts); see `guides/fonts`
- **Live Content Collections** — request-time content via `defineLiveCollection` in `src/live.config.ts`

**Removed / deprecated:** `output: 'hybrid'` is gone (use `static` default + per-route `prerender = false`, or `server` + `prerender = true`); legacy `type: 'content'` / `type: 'data'` collections are removed (use loaders in `src/content.config.ts`); `astro:schema` and `z` from `astro:content` are deprecated → import `z` from `astro/zod`.
...
- Zod 4: `import { z } from 'astro/zod'` (NOT from the `zod` package or the deprecated `astro:schema` / `astro:content`)
- Rendering: `import { render } from 'astro:content'` then `await render(entry)`
