...
Astro adds unique `data-astro-cid-*` attributes to scope styles:

```html
<!-- Output -->
<div class="container" data-astro-cid-hhnqfkh6>
  <h1 data-astro-cid-hhnqfkh6>Hello World</h1>
</div>
```
...
This installs the official `@tailwindcss/vite` plugin (Tailwind 4). The `@astrojs/tailwind` integration (Tailwind 3) is no longer the default.

Then import Tailwind into a global CSS file:
...
Then import that file in your layout so the styles apply site-wide:
...
Astro CSS rules are evaluated in a specific order of precedence: `<link>` tags in the head have the lowest precedence, followed by imported styles, and then scoped styles, which have the highest precedence. This is why a component's scoped style can override a global stylesheet rule.
