...
Hydrate once the page is done with its initial load and the `requestIdleCallback` event has fired (falling back to the document `load` event in browsers that do not support `requestIdleCallback`). Good for lower-priority interactivity.
...
```astro
<!-- timeout is the max wait in ms before hydrating, even if not yet idle -->
...
```javascript
// In browser console: each hydrated island is an <astro-island> element
document.querySelectorAll('astro-island').forEach(el => {
  console.log(el, el.getAttribute('client'), el.getAttribute('component-url'));
});
```

Note: `[data-astro-cid]` is the scoped-CSS attribute, unrelated to hydration.
