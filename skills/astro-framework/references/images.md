...
Add `priority` to load an image immediately. It sets `loading="eager"`,
`decoding="sync"`, and `fetchpriority="high"`:
...
// Entries from a glob() loader are keyed by `id` (e.g. post.id), not slug
...
### With layout (auto srcset/sizes)

With the `layout` property on the `<Image />` or `<Picture />` components,
Astro automatically generates the required `srcset` and `sizes` values and
applies the styles needed to resize the image correctly. Set it globally with
`image.layout` to apply to all image components and Markdown `![]()` images:

```astro
<Image src={hero} alt="Hero" layout="constrained" />
```

## SVG Components

Astro lets you import SVG files and use them as Astro components, inlining the
SVG content into your HTML output. Reference the default import of any local
`.svg` file. You can pass props such as `width`, `height`, `fill`, `stroke`,
and any other attribute accepted by the native `<svg>` element:
...
export default defineConfig({
  image: {
    ...
    // Allowed remote patterns (protocol-only allows all HTTPS hosts)
    remotePatterns: [
      { protocol: 'https' },
      {
        protocol: 'https',
    ...
    // Apply global styles so responsive images resize correctly.
    // Without these, images set with `layout` will not be responsive.
...
10. **Use `priority` for LCP/hero images** - Loads above-the-fold images eagerly
