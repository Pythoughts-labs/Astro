...
With `prefixDefaultLocale: true`, the home URL (`/`) is not prefixed by default and `src/pages/index.astro` is still required. Add `redirectToDefaultLocale: true` to also redirect `/` to `/[defaultLocale]/`.
...
> `redirectToDefaultLocale()`, `notFound()`, `redirectToFallback()`, `middleware()`, and `requestHasLocale()` are available only when `i18n.routing` is set to `'manual'`.
...
Requires a server-rendered project using the `@astrojs/node` or `@astrojs/vercel` adapter, with `site` configured.
...
All pages, including static prerendered pages, can access `Astro.currentLocale`. Pages rendered on demand additionally expose `Astro.preferredLocale` and `Astro.preferredLocaleList` for browser language detection:

```astro
---
const current = Astro.currentLocale;        // all pages
const preferred = Astro.preferredLocale;     // on-demand rendered pages only
const preferredList = Astro.preferredLocaleList; // on-demand rendered pages only
---
```
