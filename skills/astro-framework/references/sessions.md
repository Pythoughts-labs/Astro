...
**Requires:** on-demand rendering (an adapter)
...
- `session.cookie` — customize the session cookie. Pass a string to set just the name, or an object to override defaults (`{ name: "astro-session", sameSite: "lax", httpOnly: true, secure: true }`). The cookie is secure and httpOnly by default. (Added in `astro@5.7.0`)
- `session.ttl` — sets a global default expiration (in seconds) for session values. Override per-value with `session.set(key, value, { ttl })`.
...
| `session.set(key, value, { ttl })` | Set a value; optional `ttl` (seconds) sets per-value expiration |
