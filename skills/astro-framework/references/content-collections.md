...
> **Note:** The config file is `src/content.config.ts` and collections use a `loader`.
...
Loads entries from directories of Markdown, MDX, Markdoc, JSON, YAML, or TOML files:
...
Supports a custom `parser` for non-standard formats (e.g., CSV) or to load a single collection from a nested JSON document:
...
Custom loaders can render remote Markdown to renderable content via `renderMarkdown()` on the `LoaderContext` (added in `astro@5.9.0`), so those entries support `render()`.
...
Live collections use a different API than build-time content collections. They live in a **separate** `src/live.config.ts` file (not `src/content.config.ts`) and are defined with `defineLiveCollection()` (not `defineCollection()`). They require an adapter for on-demand rendering:
...
const products = defineLiveCollection({
  loader: productLoader({ apiKey: import.meta.env.STORE_API_KEY }),
});

export const collections = { products };
```

Query live collections with `getLiveCollection()` and `getLiveEntry()`. The page must be rendered on demand (`export const prerender = false;`). Handle errors with the runtime error classes:

```astro
---
export const prerender = false;
import { getLiveCollection, getLiveEntry } from 'astro:content';
import { LiveEntryNotFoundError, LiveCollectionValidationError } from 'astro/content/runtime';

const { entries, error } = await getLiveCollection('products');
const { entry, error: entryError } = await getLiveEntry('products', 'sku-123');

if (entryError instanceof LiveEntryNotFoundError) {
  return Astro.redirect('/404');
}
if (LiveCollectionValidationError.is(entryError)) {
...
> **Note:** `astro/zod` re-exports Zod 4. Prefer the Zod 4 top-level string format validators where available: use `z.url()` instead of `z.string().url()` and `z.email()` instead of `z.string().email()` (the chained forms are deprecated in Zod 4). The official Astro reference uses forms like `portfolio: z.url()`.
...
> **Note:** Custom validation on the `image()` helper is unsupported — do not use `image().refine(...)`.
...
### Auto-Generated JSON Schemas (data files)

Astro auto-generates a [JSON Schema](https://json-schema.org/) file for each collection (added in `astro@4.13.0`), output to `.astro/collections/<name>.schema.json`. Reference it from JSON data files via `$schema`, or point your editor at it with VS Code `json.schemas` / `yaml.schemas` settings, to get IntelliSense and type-checking for data files.
...
Import `render` from `astro:content` and call it on an entry to get a `<Content />` component:
...
> **Note:** Use `post.id` for routing params (collection entries are keyed by `id`).
