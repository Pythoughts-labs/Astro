...
Types are generated when running `astro dev` or `astro build`; run `astro sync` to generate types only.
...
**Secret validation:** By default, all secrets are validated whenever anything is imported from `astro:env/server` — even secrets you do not import. You may need to pass dummy environment variables to satisfy this validation during build, or set `env: { validateSecrets: true }` in your config to validate on start instead.
...
For the complete list of validation fields per type, see the [`envField` API reference](https://docs.astro.build/en/reference/modules/astro-config/#envfield).
...
Use `getSecret()` instead of `process.env` — its implementation is provided by your adapter, so you won't need to update calls if you switch adapters. It defaults to `process.env` in dev and build.

## Limitations

`astro:env` is a virtual module usable only inside the Astro context (middleware, routes/endpoints, Astro components, framework components, modules). It cannot be used in `astro.config.mjs` or in inline scripts — use `process.env` there instead.
