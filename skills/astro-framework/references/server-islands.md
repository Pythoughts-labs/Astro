...
Server islands run in their own isolated context. `Astro.url` and `Astro.request.url` both return the island's internal URL (e.g., `/_server-islands/Avatar`), not the page URL. For prerendered pages you also lose access to page query parameters. Use the `Referer` header to access the page URL.
...
Server island data is fetched via a GET request that passes props as an encrypted string in the URL query, so standard `Cache-Control` headers work:
...
- Server islands work in both `static` (default, with prerendered pages) and `server` output modes. Note: there is no `output: 'hybrid'` mode — its behavior is now the default of `static`.
