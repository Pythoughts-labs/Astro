...
Astro has only **two** output modes: `static` (default) and `server`. There is **no `hybrid` mode**. The old "hybrid" behavior is now the default: stay on `static`, add an adapter, and opt individual routes into on-demand rendering with `export const prerender = false`.

```
output: 'static'  (default — the right choice for almost every site)
├── Prerenders every page to HTML at build time
├── Blog, docs, landing, portfolios, marketing, e-commerce catalogs
├── Need a few request-time routes (login, dashboard, API, cart/checkout)?
│   └── Add an adapter + `export const prerender = false` on just those routes
└── Use server islands (`server:defer`) for personalized fragments so the
    surrounding page stays static and cacheable

output: 'server'  (only when MOST pages need request data)
├── Every page renders on demand; opt out per route with
│   `export const prerender = true`
├── Full SaaS/dashboard behind auth, mostly dynamic apps
└── Warning: non-prerendered pages lose static HTML / CDN caching
```

**Signs you picked wrong:**
- Builds slow/huge with `getStaticPaths` → stay `static`; move the heavy/dynamic routes to `prerender = false`
- In `server` mode but using `prerender = true` on >50% of pages → switch back to `static` (the default)
- Whole app is `server` but only 2 pages read cookies → switch to `static` + `prerender = false` on those 2
...
### New in Astro 6 (stable)

These graduated from experimental flags to stable in Astro 6 — use them directly:

- **Content Security Policy** — `security.csp` in `astro.config.mjs` auto-generates hashes for your scripts/styles → `reference/configuration-reference/#securitycsp`
- **Fonts API** — built-in font optimization for self-hosted and provider fonts, no layout shift → `guides/fonts`
- **Live Content Collections** — request-time content fetching in the content layer via `defineLiveCollection` in `src/live.config.ts` → `guides/content-collections`

Astro 6 also requires **Node 22+** (Node 18 and 20 dropped) and bundles **Zod 4** via `astro/zod` (`astro:schema` and `z` from `astro:content` are deprecated).
...
- Import Zod from `astro/zod` and render from `astro:content`
...
- Use `output: 'hybrid'` — it is not a valid mode; use `static` (default) with `prerender = false` per route, or `server` with `prerender = true`
...
Astro 6.x (Node 22+), Islands Architecture, Content Layer API (glob/file loaders, live content collections), Zod 4 Schemas (`astro/zod`), View Transitions API, Server Islands (`server:defer`), Sessions, Actions, Middleware, astro:env (type-safe environment variables), Content Security Policy (`security.csp`), Fonts API, i18n Routing, Adapters (Node, Vercel, Netlify, Cloudflare, Deno), React/Vue/Svelte/Solid integrations, Image Optimization, MDX, Markdoc, TypeScript, Scoped CSS, Tailwind CSS
