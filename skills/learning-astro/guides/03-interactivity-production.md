...
**Important**: This component is called `ClientRouter`. If the user sees errors mentioning `ViewTransitions`, that's the old name — they need `ClientRouter`.
...
├── Prerenders all pages as HTML by default
├── No server needed — deploy anywhere
├── Best performance
├── Can opt individual pages into on-demand rendering with
│   `export const prerender = false` (the former "hybrid" behavior,
│   now built into static — requires an adapter for those pages)
└── Use for: blogs, docs, portfolios, marketing sites (+ a few dynamic pages)
...
> Note: `output: 'hybrid'` no longer exists. The previous `'hybrid'` behavior is now the default, under the name `'static'`.
...
If they later want on-demand/SSR pages (a few pages with `export const prerender = false`, or `output: 'server'`):
