...
**Important**: The file is `src/content.config.ts` (at the `src/` root).
...
3. **`render(post)`**: Converts the Markdown content into a renderable `<Content />` component. Imported from `'astro:content'`.
