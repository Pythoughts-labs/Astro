...
**Cause**: Astro requires Node.js v22.12.0 or higher (odd-numbered versions like v23 are not supported)

**Solution**:
1. Check your version: `node -v`
2. If below v22.12.0, update Node.js: visit nodejs.org or use `nvm install 22`
...
**Cause**: Using the wrong config file path. The config file is `src/content.config.ts`, not `src/content/config.ts`
...
**Cause**: `entry.render()` is not a method. `render` is a standalone import from `astro:content`.
...
**Cause**: `ViewTransitions` was renamed to `ClientRouter`
...
**Solution**: Mark that page on-demand with `export const prerender = false;` (works under the default `static` output) and add an adapter. Note: `output: 'hybrid'` no longer exists — its behavior is now the default `static` output.

### Build fails with "missing adapter"

**Symptom**: Error when building with an on-demand page (e.g. `export const prerender = false`) or `output: 'server'` without an adapter
