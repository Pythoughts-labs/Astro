...
2. **What does `render()` from `astro:content` return?**
   ...
   It returns an object with a `Content` component (the rendered Markdown as an Astro component you can use in your template) and `headings` (an array of heading elements found in the content). You import `render` from `'astro:content'` and call it as `render(entry)`.
   </details>
...
**Cause**: `render` is a standalone import, not a method on the entry.
