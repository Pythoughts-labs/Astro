...
1. **What is the difference between `static` and `server` output modes?**
   <details>
   <summary>Answer</summary>

   **Static** (default): Pages are pre-built as HTML files at build time. No server needed — deploy anywhere. Best performance. You can still opt individual pages into on-demand rendering by adding `export const prerender = false` (plus an adapter), so a mostly-static site can have a few dynamic pages (login, dashboard, API routes). (The old `'hybrid'` mode no longer exists — this default `'static'` behavior replaced it.)
...
2. **When do you need a deployment adapter?**
   ...
   Whenever any page is rendered on demand — `output: 'server'`, or the default `static` output with a page marked `export const prerender = false` — these need a server runtime to handle on-demand rendering. Fully static sites don't need an adapter because they're just HTML files that any web server can serve. Adapters are platform-specific: `@astrojs/netlify`, `@astrojs/vercel`, `@astrojs/node`, `@astrojs/cloudflare`.
   </details>
...
**Fix**: Either remove the request-time code, or mark the page for on-demand rendering: `export const prerender = false;` (requires an adapter; works under the default static output).

### Build error: "missing adapter"

**Cause**: Using `output: 'server'` (or any on-demand page) without installing an adapter.
...
- Try marking one page as `export const prerender = false` and adding an adapter (`npx astro add netlify`/vercel/node)
...
**Where to go next:**
- Explore on-demand rendering (`export const prerender = false`) for dynamic features
