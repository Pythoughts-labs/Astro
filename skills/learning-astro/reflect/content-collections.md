...
3. **Why is the config file `src/content.config.ts` and not `src/content/config.ts`?**
   ...
   The config file is `src/content.config.ts` (at the `src/` root), part of the Content Layer API. Using `src/content/config.ts` will cause collections not to be found.
   </details>
