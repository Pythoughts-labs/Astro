...
On an on-demand-rendered route (a page with `export const prerender = false`, or with `output: 'server'`), dynamic routes without `getStaticPaths()` are rendered on demand per request.
