...
Collections are defined in `src/content.config.ts` (at the `src/` root):
...
Rendering uses a top-level `render()` function:
...
3. **Forgetting `render`**: import `render` from `astro:content`, then call `render(entry)`
4. **Missing loader**: collections require a `loader` property — there's no implicit file loading
